...
- Отвечай только валидным JSON, никаких пояснений до или после.
    ...
    suspend fun verifySolution(
        ...
        val prompt = buildVerificationPrompt(symptoms, errorCodes, solutionText)
    ...
    private fun buildVerificationPrompt(symptoms: String, errorCodes: String, solutionText: String): String {
        return """
            Ты - главный эксперт-автоэлектрик. Твоя задача оценить достоверность предложенного решения от пользователя.
            ...
            Оцени, насколько это решение логично, безопасно и вероятно решит проблему. 
    ...
    private fun parseVerificationResult(jsonText: String): AiVerificationResult? {
        return try {
            ...
            gson.fromJson(jsonText, AiVerificationResult::class.java)
        } catch (e: Exception) {
